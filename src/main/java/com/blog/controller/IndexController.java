...
public class IndexController {
    ...
    public String toIndex(@RequestParam(required = false,defaultValue = "1",value = "pagenum")int pagenum, Model model){
        ...
        model.addAttribute("recommendBlogs", blogService.findByViews());
        try{
            cache.cacheBlog();
        }catch (Exception e)
        {
    ...
    @GetMapping("/blog/redis/{id}")
    public String redisBlog(@PathVariable String id, Model model)
    {

        model.addAttribute("blog", cache.getBlog(id));
        return "blog";
    }
